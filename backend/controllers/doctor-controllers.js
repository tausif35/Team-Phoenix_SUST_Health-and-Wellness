...
exports.login = catchAsync(async (req, res, next) => {
    ...
    const existingDoctor = await Doctor.findOne({ email: email });
    if (!existingDoctor) throw new HttpError("Invalid credentials, could not log you in.", 401)
    const isValidPassword = await bcrypt.compare(password, existingDoctor.password);
